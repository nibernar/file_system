...
class FileSystemConfigValidation {
  ...
  @Transform(({ value }) => {
    if (typeof value === 'string' && value.length < 32) {
      throw new Error('SECURITY_TOKEN_SECRET must be at least 32 characters long');
    }
    return value;
  })
...
function validateConfig(config: Record<string, unknown>): FileSystemConfigValidation {
  ...
  for (const field of requiredStringFields) {
    if (!config[field] || config[field] === '') {
      throw new Error(`Configuration validation failed: ${field} is required and cannot be empty`);
    }
  }
...
export default registerAs('fileSystem', (): FileSystemConfig => {
  ...
  const config: FileSystemConfig = {
    ...
    cdn: {
      ...
      cacheControl: validatedEnv.CDN_CACHE_CONTROL || DEFAULT_CONFIG.cdn!.cacheControl!,
      invalidationToken: validatedEnv.CDN_INVALIDATION_TOKEN,
      edgeLocations: validatedEnv.CDN_EDGE_LOCATIONS || DEFAULT_CONFIG.cdn!.edgeLocations!,
      defaultTtl: validatedEnv.CDN_DEFAULT_TTL || DEFAULT_CONFIG.cdn!.defaultTtl!,
      maxTtl: validatedEnv.CDN_MAX_TTL || DEFAULT_CONFIG.cdn!.maxTtl!
    },
    
    processing: {
      maxFileSize: validatedEnv.MAX_FILE_SIZE || DEFAULT_CONFIG.processing!.maxFileSize!,
      allowedMimeTypes: validatedEnv.ALLOWED_MIME_TYPES || DEFAULT_CONFIG.processing!.allowedMimeTypes!,
      virusScanTimeout: validatedEnv.VIRUS_SCAN_TIMEOUT || DEFAULT_CONFIG.processing!.virusScanTimeout!,
      imageOptimizationQuality: validatedEnv.IMAGE_OPTIMIZATION_QUALITY || DEFAULT_CONFIG.processing!.imageOptimizationQuality!,
      thumbnailSize: validatedEnv.THUMBNAIL_SIZE || DEFAULT_CONFIG.processing!.thumbnailSize!,
      pdfCompressionLevel: validatedEnv.PDF_COMPRESSION_LEVEL || DEFAULT_CONFIG.processing!.pdfCompressionLevel!,
      maxWorkers: validatedEnv.PROCESSING_MAX_WORKERS || DEFAULT_CONFIG.processing!.maxWorkers!,
      chunkSize: validatedEnv.PROCESSING_CHUNK_SIZE || DEFAULT_CONFIG.processing!.chunkSize!
    },
    
    security: {
      presignedUrlExpiry: validatedEnv.PRESIGNED_URL_EXPIRY || DEFAULT_CONFIG.security!.presignedUrlExpiry!,
      maxPresignedUrls: validatedEnv.MAX_PRESIGNED_URLS || DEFAULT_CONFIG.security!.maxPresignedUrls!,
      // Gestion booléens : undefined = défaut, 'true' = true, tout le reste = false
      ipRestrictionEnabled: process.env.IP_RESTRICTION_ENABLED === undefined ? DEFAULT_CONFIG.security!.ipRestrictionEnabled! :
                           process.env.IP_RESTRICTION_ENABLED === 'true',
      scanVirusEnabled: process.env.SCAN_VIRUS_ENABLED === undefined ? DEFAULT_CONFIG.security!.scanVirusEnabled! :
                       process.env.SCAN_VIRUS_ENABLED === 'true',
      rateLimitUploadsPerMinute: validatedEnv.RATE_LIMIT_UPLOADS_PER_MINUTE || DEFAULT_CONFIG.security!.rateLimitUploadsPerMinute!,
      abuseBlockDuration: validatedEnv.ABUSE_BLOCK_DURATION || DEFAULT_CONFIG.security!.abuseBlockDuration!,
      deviceFingerprintingEnabled: process.env.DEVICE_FINGERPRINTING_ENABLED === undefined ? DEFAULT_CONFIG.security!.deviceFingerprintingEnabled! :
    ...
    case 'development':
      return {
        ...
        processing: {
          ...
          imageOptimizationQuality: baseConfig.processing!.imageOptimizationQuality,
        ...
        security: {
          ...
          rateLimitUploadsPerMinute: baseConfig.security!.rateLimitUploadsPerMinute,
    ...
    case 'staging':
      return {
        ...
        security: {
          ...
          abuseBlockDuration: baseConfig.security!.abuseBlockDuration,
          deviceFingerprintingEnabled: baseConfig.security!.deviceFingerprintingEnabled
        }
      };
