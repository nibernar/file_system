...
export class RateLimitService {
  ...
  async incrementCounter(
    ...
    try {
      const { key } = this.getRateLimitConfig(identifier, {
        endpoint: operation,
      });
      ...
      if (newCount >= limit * 0.8) {
        this.logger.warn(
          `Rate limit warning: ${key} at ${newCount}/${limit} (${Math.round((newCount / limit) * 100)}%)`,
        );
      }
    } catch (error) {
      this.logger.error(
        `Rate limit increment error: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async resetCounter(identifier: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Rate limit reset error: ${error.message}`,
        error.stack,
      );
    }
  }
    ...
    try {
      ...
      return {
        ...
        limits: {
          ...
          ip: FILE_SYSTEM_CONSTANTS.SECURITY_LIMITS
            .MAX_UPLOADS_PER_IP_PER_MINUTE,
        },
      };
    } catch (error) {
      this.logger.error(
        `Rate limit stats error: ${error.message}`,
        error.stack,
      );
      return {
        userCount: 0,
  ...
  private getRateLimitConfig(
    ...
    if (this.isIpAddress(identifier)) {
      return {
        ...
        limit:
          FILE_SYSTEM_CONSTANTS.SECURITY_LIMITS.MAX_UPLOADS_PER_IP_PER_MINUTE,
      };
    }
