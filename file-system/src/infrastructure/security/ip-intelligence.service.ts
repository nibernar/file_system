...
export class IpIntelligenceService {
  ...
  async getIpIntelligence(ip: string): Promise<IpIntelligence> {
    try {
      ...
      this.logger.log(`IP intelligence generated for ${ip}: ${JSON.stringify({
  ...
  async recordIpActivity(ip: string, event: string): Promise<void> {
    try {
      ...
      if (count > 100) {
        this.logger.warn(`High activity detected from IP ${ip}: ${count} ${event} events`);
      }
  ...
  private async getGeoData(ip: string): Promise<any> {
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.get<GeoApiResponse>(`http://ip-api.com/json/${ip}?fields=status,message,continent,continentCode,country,countryCode,region,regionName,city,district,zip,lat,lon,timezone,offset,currency,isp,org,as,asname,reverse,mobile,proxy,hosting,query`)
      );
  ...
  private async updateTorExitNodes(): Promise<void> {
    try {
      ...
      this.logger.log(`Updated Tor exit nodes list: ${this.TOR_EXIT_NODES.size} nodes`);
