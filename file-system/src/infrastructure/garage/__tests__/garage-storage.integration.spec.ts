...
describe('GarageStorageService Integration', () => {
  ...
  const integrationConfig: FileSystemConfig = {
    garage: {
      ...
      secretKey: process.env.GARAGE_TEST_SECRET_KEY || 'abcdef1234567890abcdef1234567890abcdef12',
      buckets: {
        documents: process.env.GARAGE_TEST_BUCKET || `test-integration-${Date.now()}`,
  ...
  beforeAll(async () => {
    ...
    console.log(`✅ Connected to Garage S3 at ${integrationConfig.garage.endpoint}`);
  ...
  const generateTestKey = (prefix: string = 'integration-test', extension: string = 'bin'): string => {
  ...
  describe('Complete CRUD Operations on Real Garage Instance', () => {
    it('should perform complete CRUD operations on real Garage instance', async () => {
      ...
      const uploadResult = await service.uploadObject(objectKey, testContent, metadata);
      ...
      console.log(`✅ Metadata retrieval successful - Size: ${objectInfo.size} bytes`);
      ...
      createdObjects = createdObjects.filter(key => key !== objectKey && key !== copyKey);
    }, 30000);
    ...
    it('should handle objects of various sizes correctly', async () => {
      ...
      for (const testCase of testSizes) {
        console.log(`🧪 Testing ${testCase.name} file (${testCase.size} bytes)`);
        ...
        if (testCase.size > 1024 * 1024) {
          console.log(`  📊 Throughput: ${(testCase.size / 1024 / 1024 / (uploadDuration / 1000)).toFixed(2)} MB/s upload`);
        }
      }
    }, 60000);
  });
  ...
  describe('Concurrent Operations Handling', () => {
    it('should handle concurrent uploads correctly', async () => {
      ...
      const uploadTasks = Array.from({ length: concurrentCount }, (_, index) => ({
      ...
      console.log(`✅ All ${concurrentCount} uploads completed in ${totalDuration}ms`);
      console.log(`📊 Average time per upload: ${(totalDuration / concurrentCount).toFixed(2)}ms`);

      const sampleIndices = [0, Math.floor(concurrentCount / 2), concurrentCount - 1];
      
      for (const index of sampleIndices) {
        const downloadResult = await service.downloadObject(uploadTasks[index].key);
        expect(downloadResult.body.equals(uploadTasks[index].content)).toBe(true);
      }
    ...
    it('should handle mixed concurrent operations correctly', async () => {
      const setupObjects = Array.from({ length: 5 }, (_, index) => ({
        ...
        content: Buffer.from(`Setup content for mixed operations test ${index}`)
      }));
      ...
      const mixedOperations = [
        ...Array.from({ length: 3 }, (_, i) => 
          service.uploadObject(
            ...
            { contentType: 'application/octet-stream', userId: 'mixed-ops-user' }
          )
        ),
        
        ...setupObjects.slice(0, 3).map(obj => 
          service.downloadObject(obj.key)
        ),
        
        ...setupObjects.slice(0, 2).map(obj => 
          service.getObjectInfo(obj.key)
        ),
      ...
      console.log(`✅ Mixed operations completed: ${successful} successful, ${failed} failed`);
  ...
  describe('Data Integrity and Reliability', () => {
    it('should maintain data integrity across operations', async () => {
      ...
      const testFiles = [
        {
          ...
          content: Buffer.from('This is a text file for integrity testing with special chars: àéèñü'),
          contentType: 'text/plain'
        },
        {
      ...
      const originalChecksums: TestFile[] = testFiles.map(file => ({
        ...file,
        md5: require('crypto').createHash('md5').update(file.content).digest('hex'),
        sha256: require('crypto').createHash('sha256').update(file.content).digest('hex')
      }));
      ...
      for (const file of originalChecksums) {
        ...
        const uploadResult = await service.uploadObject(key, file.content, metadata);
      ...
      for (const file of uploadResults) {
        ...
        console.log(`  ✅ ${file.name}: integrity verified (${downloadResult.body.length} bytes)`);
      }
    ...
    it('should handle network interruptions gracefully', async () => {
      ...
      try {
        const uploadResult = await service.uploadObject(key, testContent, metadata);
        expect(uploadResult).toBeDefined();
        ...
        console.warn(`⚠️ Network test failed (expected in some environments):`, error.message);
      }
    }, 30000);
    ...
    it('should handle edge cases and limits correctly', async () => {
      ...
      const specialCharsKey = generateTestKey('edge-special-chars-àéèñü', 'txt');
      const specialContent = Buffer.from('Content with special characters: àáâãäåæçèéêë');
      
      const specialResult = await service.uploadObject(specialCharsKey, specialContent, {
      ...
      const complexMetadataKey = generateTestKey('edge-complex-metadata', 'json');
      ...
      const complexResult = await service.uploadObject(complexMetadataKey, complexContent, complexMetadata);
      const complexInfo = await service.getObjectInfo(complexMetadataKey);
      
      expect(complexInfo.customMetadata['user-id']).toBe(complexMetadata.userId);
      expect(complexInfo.customMetadata['project-id']).toBe(complexMetadata.projectId);
      console.log(`  ✅ Complex metadata handled correctly`);

      const longKey = generateTestKey('edge-very-long-name-' + 'x'.repeat(200), 'bin');
      const longKeyContent = Buffer.from('Long key test content');
      
      try {
        const longKeyResult = await service.uploadObject(longKey, longKeyContent, {
          contentType: 'application/octet-stream',
          userId: 'edge-test-user'
        });
        ...
        console.log(`  ℹ️ Long key rejected (implementation limit): ${error.message}`);
      }
  ...
  describe('Performance and Monitoring', () => {
    it('should meet basic performance benchmarks', async () => {
      ...
      const smallUploadResult = await service.uploadObject(smallKey, smallContent, {
        contentType: 'application/octet-stream',
      ...
      console.log(`  ✅ Small file upload: ${smallUploadTime}ms (${benchmarks.smallFileUpload.size} bytes)`);
      ...
      expect(mediumUploadTime).toBeLessThan(benchmarks.mediumFileUpload.maxTime);
      console.log(`  ✅ Medium file upload: ${mediumUploadTime}ms (${benchmarks.mediumFileUpload.size} bytes)`);
      ...
      expect(smallDownloadTime).toBeLessThan(benchmarks.smallFileDownload.maxTime);
      ...
      const smallThroughput = (benchmarks.smallFileUpload.size / 1024 / 1024) / (smallUploadTime / 1000);
      const mediumThroughput = (benchmarks.mediumFileUpload.size / 1024 / 1024) / (mediumUploadTime / 1000);
      
      console.log(`📊 Upload throughput: Small=${smallThroughput.toFixed(2)} MB/s, Medium=${mediumThroughput.toFixed(2)} MB/s`);
    }, 45000);
    ...
    it('should provide accurate bucket information', async () => {
      ...
      console.log(`📊 Usage: ${bucketInfo.usage.objectCount} objects, ${(bucketInfo.usage.totalSize / 1024 / 1024).toFixed(2)} MB`);
    }, 15000);
  });
});