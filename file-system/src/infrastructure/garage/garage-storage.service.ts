...
export interface IGarageStorageService {
  ...
  uploadObject(key: string, buffer: Buffer, metadata: ObjectMetadata): Promise<UploadResult>;
  ...
  initializeMultipartUpload(key: string, metadata: ObjectMetadata): Promise<MultipartUpload>;
  uploadPart(uploadId: string, partNumber: number, buffer: Buffer): Promise<PartUploadResult>;
  completeMultipartUpload(uploadId: string, parts: CompletedPart[]): Promise<UploadResult>;
...
export class GarageStorageService implements IGarageStorageService {
  ...
  constructor(
    ...
    this.logger.log(`Garage Storage Service initialized with endpoint: ${config.garage.endpoint}`);
  }
   ...
   * ```
   */
  async uploadObject(key: string, buffer: Buffer, metadata: ObjectMetadata): Promise<UploadResult> {
    ...
    try {
      ...
      if (buffer.length > 100 * 1024 * 1024) { // 100MB
        this.logger.log(`Using multipart upload for large file: ${key} (${buffer.length} bytes)`);
   ...
   * ```
   */
  async initializeMultipartUpload(key: string, metadata: ObjectMetadata): Promise<MultipartUpload> {
    try {
      ...
      this.logger.log(`Multipart upload initialized for key: ${key}, uploadId: ${response.UploadId}`);
      ...
      this.logger.error(`Initialize multipart upload failed for key: ${key}`, error);
   ...
   * ```
   */
  async uploadPart(uploadId: string, partNumber: number, buffer: Buffer): Promise<PartUploadResult> {
    try {
      ...
      this.logger.log(`Part ${partNumber} uploaded successfully for uploadId: ${uploadId}`);
      ...
      this.logger.error(`Upload part failed for uploadId: ${uploadId}, part: ${partNumber}`, error);
   ...
   * ```
   */
  async completeMultipartUpload(uploadId: string, parts: CompletedPart[]): Promise<UploadResult> {
    try {
      ...
      const totalSize = parts.reduce((total, part) => total + (part.size || 0), 0);

      this.logger.log(`Multipart upload completed successfully for uploadId: ${uploadId}`);
      ...
      return {
        ...
        storageKey: key,
        etag: response.ETag.replace(/"/g, ''),
        location: response.Location || `${config.garage.endpoint}/${this.defaultBucket}/${key}`,
        metadata: fileMetadata,
        uploadDuration: 0 // À calculer si nécessaire
      };

    } catch (error) {
      this.logger.error(`Complete multipart upload failed for uploadId: ${uploadId}`, error);
  ...
  async abortMultipartUpload(uploadId: string): Promise<void> {
    try {
      ...
      this.logger.log(`Multipart upload aborted successfully for uploadId: ${uploadId}`);

    } catch (error) {
      this.logger.error(`Abort multipart upload failed for uploadId: ${uploadId}`, error);
   ...
   * ```
   */
  async copyObject(sourceKey: string, destinationKey: string): Promise<CopyResult> {
    try {
      ...
      this.logger.log(`Object copied successfully from ${sourceKey} to ${destinationKey}`);
      ...
      this.logger.error(`Copy object failed from ${sourceKey} to ${destinationKey}`, error);
   ...
   */
  private async uploadLargeObject(key: string, buffer: Buffer, metadata: ObjectMetadata, uploadId: string): Promise<UploadResult> {
    const startTime = Date.now();
   ...
   */
  private validateUploadParams(key: string, buffer: Buffer, metadata: ObjectMetadata): void {
    ...
    if (buffer.length > maxSize) {
      throw new Error(`File too large: ${buffer.length} bytes exceeds maximum ${maxSize} bytes`);
    }
  }
