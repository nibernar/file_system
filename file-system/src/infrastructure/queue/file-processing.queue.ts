...
export class QueueUtils {
   ...
   */
  static adjustPriorityForLoad(basePriority: number, queueSize: number): number {
  ...
  static validateJobOptions(
    ...
    const defaults = DEFAULT_JOB_OPTIONS[jobType] || DEFAULT_JOB_OPTIONS[JOB_TYPES.PROCESS_UPLOADED_FILE];
    
    return {
      ...defaults,
      ...options,
      // S'assurer que certaines valeurs restent dans des limites raisonnables
      priority: Math.max(0, Math.min(10, options.priority || defaults.priority || 5)),
      attempts: Math.max(1, Math.min(10, options.attempts || defaults.attempts || 3)),
      timeout: Math.max(1000, Math.min(3600000, options.timeout || defaults.timeout || 60000))
    };
  }
}
