...
export class DocumentProcessorService {
  ...
  async processDocument(
    ...
    try {
      ...
      const config = {
        ...
        extractSpecializedMetadata:
          options.extractSpecializedMetadata !== false,
      ...
      try {
        if (iconv.encodingExists(detectedEncoding)) {
          ...
          this.logger.warn(
            `Encodage ${detectedEncoding} non supporté, fallback UTF-8`,
          );
          textContent = sourceBuffer.toString('utf8');
        }
      } catch (encodingError) {
        this.logger.warn(
          `Erreur décodage ${detectedEncoding}, fallback UTF-8: ${encodingError.message}`,
        );
        textContent = sourceBuffer.toString('utf8');
      }
      ...
      if (config.detectLanguage && textContent.length > 50) {
        try {
          ...
          this.logger.debug(
            `Échec détection langage ${fileId}: ${langError.message}`,
          );
      ...
      if (config.validateStructure) {
        try {
          ...
          this.logger.debug(
            `Échec validation structure ${fileId}: ${structError.message}`,
          );
      ...
      if (
        ...
        try {
          ...
          this.logger.debug(
            `Échec génération résumé ${fileId}: ${summaryError.message}`,
          );
      ...
      if (config.extractSpecializedMetadata) {
        try {
          ...
          this.logger.debug(
            `Échec extraction métadonnées spécialisées ${fileId}: ${metaError.message}`,
          );
      ...
      if (
        ...
        try {
          const optimizedBuffer = iconv.encode(
            textContent,
            config.targetEncoding,
          );
          ...
          this.logger.debug(
            `Encodage optimisé ${fileId}: ${detectedEncoding} → ${config.targetEncoding}`,
          );
        } catch (optimizeError) {
          this.logger.debug(
            `Échec optimisation encodage ${fileId}: ${optimizeError.message}`,
          );
      ...
      this.logger.error(
        `Échec traitement document ${fileId} après ${processingTime}ms: ${error.message}`,
      );
  ...
  private async validateDocumentStructure(
    content: string,
    fileId: string,
  ): Promise<DocumentStructureValidation> {
    ...
    try {
      ...
      if (content.trim().startsWith('{') && content.trim().endsWith('}')) {
        ...
        if (inconsistentRows > 0) {
          result.warnings.push(
            `${inconsistentRows} ligne(s) avec nombre de colonnes incohérent`,
          );
        }
      ...
      this.logger.debug(
        `Structure validée pour ${fileId}: ${result.detectedFormat}, valide: ${result.valid}`,
      );
    } catch (error) {
      this.logger.warn(
        `Erreur validation structure ${fileId}: ${error.message}`,
      );
  ...
  private async extractSpecializedMetadata(
    ...
    try {
        ...
        default:
          ...
          metadata.textStructure = {
            ...
            avgWordsPerLine:
              content
                .split('\n')
                .reduce((acc, line) => acc + line.split(/\s+/).length, 0) /
              content.split('\n').length,
      ...
      this.logger.debug(
        `Erreur extraction métadonnées spécialisées: ${error.message}`,
      );
    }
  ...
  private async getDocumentBuffer(fileId: string): Promise<Buffer> {
    try {
      ...
      this.logger.error(
        `Échec récupération document ${fileId}: ${error.message}`,
      );
  ...
  private calculateObjectDepth(obj: any): number {
    ...
    if (Array.isArray(obj)) {
      return (
        1 + Math.max(0, ...obj.map((item) => this.calculateObjectDepth(item)))
      );
    }
    ...
    return (
      ...
      Math.max(
        0,
        ...Object.values(obj).map((value) => this.calculateObjectDepth(value)),
      )
    );
  }
