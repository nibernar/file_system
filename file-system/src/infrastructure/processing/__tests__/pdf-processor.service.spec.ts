...
describe('PdfProcessorService', () => {
   ...
   */
  const createSequentialSpawnMock = (sequence: Array<{
    command?: string;
    exitCode?: number;
    stdout?: string;
    stderr?: string;
  ...
  beforeEach(async () => {
    ...
    mockSpawn.mockImplementation((command: string) => {
      if (command.includes('version') || command === 'gs' || command === 'pdfinfo') {
...
Page size: 612 x 792 pts (letter)`
      ...
      expect(result.compressionRatio).toBe(optimizedSize / sourcePdfBuffer.length);
      expect(result.techniques).toEqual(
        expect.arrayContaining(['pdf_compression', 'image_optimization', 'linearization'])
      );
      expect(result.pageCount).toBe(5);

      // Vérification des appels
      expect(mockSpawn).toHaveBeenCalledWith('pdfinfo', expect.any(Array));
      expect(mockSpawn).toHaveBeenCalledWith('gs', expect.arrayContaining([
        '-sDEVICE=pdfwrite'
      ]));
      expect(mockSpawn).toHaveBeenCalledWith('qpdf', expect.arrayContaining([
        '--linearize'
      ]));
    });
    ...
    it('should handle Ghostscript errors gracefully', async () => {
      ...
      // Act & Assert
      await expect(service.optimizePdf(fileId, {}))
...
Page size: 612 x 792 pts (letter)`
      ...
      // Mock lecture images
      const pageBuffers = Array(pageCount).fill(null).map(() => createTestJPEGBuffer());
      pageBuffers.forEach(buffer => {
        mockFs.readFile.mockResolvedValueOnce(buffer);
      });

      // Mock sauvegarde
      const uploadPromises = Array(pageCount).fill(null).map((_, i) => 
    ...
    it('should validate preview generation parameters', async () => {
      ...
      await expect(service.generatePreview(fileId, 3, 500))
        .rejects.toThrow(ThumbnailGenerationException);

      await expect(service.generatePreview(fileId, 3, 50))
        .rejects.toThrow(ThumbnailGenerationException);

      await expect(service.generatePreview(fileId, 0, 150))
        .rejects.toThrow(ThumbnailGenerationException);

      await expect(service.generatePreview(fileId, 15, 150))
        .rejects.toThrow(ThumbnailGenerationException);
    });
  });
...
Page size:      612 x 792 pts (letter)`;
    ...
    it('should handle pdfinfo command failures', async () => {
      ...
      // Act & Assert
      await expect(service.extractMetadata(fileId))
  ...
  describe('Robustesse et Gestion d\'Erreurs', () => {
    it('should handle missing PDF files gracefully', async () => {
      ...
      await expect(service.optimizePdf(nonExistentFileId, {}))
        .rejects.toThrow(OptimizationException);

      // Vérifier qu'aucun traitement PDF n'a été tenté
      const processedCalls = mockSpawn.mock.calls.filter(call => 
