...
const setupTestEnvironment = (): void => {
  const complementaryEnvVars = {
    ...
    GARAGE_ACCESS_KEY:
      process.env.GARAGE_ACCESS_KEY || 'GK_TEST_ACCESS_KEY_123456789',
    GARAGE_SECRET_KEY:
      process.env.GARAGE_SECRET_KEY ||
      'test_secret_key_with_sufficient_length_for_aws_sdk_validation_requirements',
    GARAGE_BUCKET_DOCUMENTS:
      process.env.GARAGE_BUCKET_DOCUMENTS || 'test-coders-documents',
    GARAGE_BUCKET_BACKUPS:
      process.env.GARAGE_BUCKET_BACKUPS || 'test-coders-backups',
    ...
    GARAGE_TEST_ACCESS_KEY:
      process.env.GARAGE_TEST_ACCESS_KEY || 'GK1234567890ABCDEFGH',
    GARAGE_TEST_SECRET_KEY:
    ...
    CDN_INVALIDATION_TOKEN:
      process.env.CDN_INVALIDATION_TOKEN ||
      'test_cdn_invalidation_token_123456789',
    CDN_EDGE_LOCATIONS:
      process.env.CDN_EDGE_LOCATIONS || 'eu-west-1,us-east-1,ap-southeast-1',
    ...
    RATE_LIMIT_UPLOADS_PER_MINUTE:
      process.env.RATE_LIMIT_UPLOADS_PER_MINUTE || '20',
    ABUSE_BLOCK_DURATION: process.env.ABUSE_BLOCK_DURATION || '60',
    DEVICE_FINGERPRINTING_ENABLED:
      process.env.DEVICE_FINGERPRINTING_ENABLED || 'false',
    SECURITY_TOKEN_SECRET:
      process.env.SECURITY_TOKEN_SECRET ||
      'test_security_token_secret_with_minimum_32_characters_length_for_jwt_validation',
    ...
    DATABASE_URL:
      ...
      'postgresql://test:test@localhost:5432/test_coders_file_system',
    REDIS_URL: process.env.REDIS_URL || 'redis://localhost:6379/15',

    // Configuration Authentification (intégration C-05)
    JWT_SECRET:
      process.env.JWT_SECRET ||
      'test_jwt_secret_for_file_system_tests_with_sufficient_length',
  ...
  console.log(
    '✅ Test environment variables configured for C-06 File System (respecting .env.test)',
  );
  console.log(
    `📝 Using configuration: MAX_FILE_SIZE=${process.env.MAX_FILE_SIZE}, GARAGE_ENDPOINT=${process.env.GARAGE_ENDPOINT}`,
  );
};
 ...
 */
export function createTestFileBuffer(
 ...
 */
export function createTestJSONBuffer(
  data: any = { test: true, timestamp: new Date().toISOString() },
): Buffer {
 ...
 */
export function generateTestStorageKey(
  prefix: string = 'test-files',
  filename?: string,
): string {
  const name =
    filename ||
    `test-file-${Date.now()}-${Math.random().toString(36).substring(7)}`;
  return `${prefix}/${name}`;
}
...
expect.extend({
  ...
  toBeValidFileSize(received: number) {
    ...
    const pass =
      typeof received === 'number' && received > 0 && received <= maxSize;
  ...
  toBeValidChecksum(received: string, algorithm: 'md5' | 'sha256') {
    ...
    const pass =
      typeof received === 'string' && patterns[algorithm].test(received);
  ...
  toBeValidPresignedUrl(received: string) {
    try {
      ...
      const pass =
        hasSignature && (url.protocol === 'https:' || url.protocol === 'http:');
  ...
  toHaveValidFileStructure(received: any) {
    ...
    const hasRequiredFields = requiredFields.every(
      (field) => received && typeof received === 'object' && field in received,
    );
