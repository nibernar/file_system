...
export class FileAuditInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    this.logger.debug(`Starting audit for ${baseAuditEvent.method} ${baseAuditEvent.endpoint}`, {
      auditEventId,
  ...
  private extractActionFromContext(context: ExecutionContext): string {
    ...
    return actionMap[methodName] || `${className.toUpperCase()}_${methodName.toUpperCase()}`;
  }
   ...
   * @private
   */
  private extractSecurityContext(request: AuthenticatedRequest): FileAuditEvent['securityContext'] {
    if (!request.security) {
      return undefined;
    }
   ...
   * @private
   */
  private calculateRequestSize(request: AuthenticatedRequest): number | undefined {
  ...
  private determineErrorLogLevel(error: any): 'error' | 'warn' | 'log' {
    ...
    if (error.name?.includes('Security') || error.name?.includes('Unauthorized')) {
      return 'error';
    }
  ...
  private isSuspiciousError(
    ...
    if (auditEvent.userAgent === 'Unknown' || auditEvent.userAgent.includes('bot')) {
      return true;
    }
