...
export class FileAccessGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      if (!user || !user.id) {
        this.logger.warn(`Access attempt without authentication`, { requestId });
        throw new UnauthorizedException('Authentication required');
      }

      // 3. Extraction de l'opération requise via décorateur
      const requiredOperation = this.reflector.get<FileOperation>(FILE_OPERATION_KEY, handler);
  ...
  private isValidFileId(fileId: string): boolean {
    ...
    if (fileId.length < 8) {  // Changé de 3 à 8
      return false;
    }
  ...
  private extractSecurityContext(request: AuthenticatedRequest): {
    ...
    return {
      ipAddress: request.security?.clientIp || 
                 request.ip || 
   ...
   * @private
   */
  private isUserAdmin(user: NonNullable<AuthenticatedRequest['user']>): boolean {
  ...
  private sanitizeLogData(data: any): any {
    ...
    if (sanitized.fileId && typeof sanitized.fileId === 'string') {
      sanitized.fileId = sanitized.fileId.length > 20 
